...
class StickyNotes:
  ...
  def write_notes(self, *notes):
    if len(notes) >= 4:
      warnings.warn("For an unknown reason adding more than 3 notes at a single time will cause the notes list to stop working")
    ...
    mapped_notes = list(map(lambda note: (note.text, self.get_window_position_string(note), note.theme, note.get_is_open(), note.id, note.get_ticks(), note.get_ticks()), notes))
  ...
  class __Theme:
    """colours the sticky note can be
    """
    def __init__(self):
      """initilizes the colours
      """
      self.themes = [
        "Yellow",
        "White",
